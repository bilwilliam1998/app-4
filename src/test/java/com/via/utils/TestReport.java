...
public class TestReport extends Base implements ITestListener {
	...
	public void onTestSuccess(ITestResult result) {
		...
		try {
			...
			String path = HelperFunctions.getSnap(".\\src\\test\\resources\\ScreenshotResult\\via_com"+ newDate + ".png");
	...
	public void onTestFailure(ITestResult result) {
		...
		try {
			testLog.info("PFA : Final Snapshot Below ");
			String newDate = new Date().toString().replaceAll(" ", "_").replaceAll(":", "_");
			String path = HelperFunctions.getSnap(".\\src\\test\\resources\\ScreenshotResult\\"+"via.com"+ newDate + ".png");
			testLog.addScreenCaptureFromPath(path);
		} catch (IOException e) {
			e.printStackTrace();
		}
